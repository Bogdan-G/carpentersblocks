...
public class CbBakedModel implements IBakedModel {

    public static final ModelResourceLocation BAKED_MODEL = new ModelResourceLocation(CarpentersBlocks.MODID + ":block/bakedmodelblock");

    private TextureAtlasSprite sprite;
    private VertexFormat format;

    public CbBakedModel(IModelState state, VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
        this.format = format;
        ResourceLocation str0 = new ResourceLocation(CarpentersBlocks.MODID, "blocks/full_frame");
        sprite = bakedTextureGetter.apply(str0);
        int u = 0;
    }
    ...
    public List<BakedQuad> getQuads(IBlockState state, EnumFacing side, long rand) {
        ...
        Boolean north = extendedBlockState.getValue(CarpentersBlock.NORTH);
        Boolean south = extendedBlockState.getValue(CarpentersBlock.SOUTH);
        Boolean west = extendedBlockState.getValue(CarpentersBlock.WEST);
        Boolean east = extendedBlockState.getValue(CarpentersBlock.EAST);
        Boolean up = extendedBlockState.getValue(CarpentersBlock.UP);
        Boolean down = extendedBlockState.getValue(CarpentersBlock.DOWN);
